...
RSpec.describe HelpRequestCallsController, type: :controller do
  fixtures :all
  ...
  describe "GET new" do
    ...
    context "for an authenticated nurse" do
      ...
      it "sets the help_request_call" do
        sign_in_user nurse

        get :new, participant_id: participant.id, locale: locale

        expect(assigns(:help_request_call))
          .to be_instance_of HelpRequestCall
        expect(assigns(:help_request_call).participant)
          .to eq participant
      end
    end
  end
  ...
  describe "POST create" do
    ...
    context "for an authenticated nurse" do
      context "when successful" do
        it "creates a new HelpRequestCall" do
          sign_in_user nurse

          expect do
            post :create, participant_id: participant.id,
                          help_request_call: valid_params,
                          locale: locale
          end.to change {
            HelpRequestCall.where(participant: participant).count
          }.by(1)
        end
      ...
      context "when unsuccessful" do
        it "sets the flash alert" do
          ...
          post :create, participant_id: participant.id, locale: locale,
                        help_request_call: invalid_params
