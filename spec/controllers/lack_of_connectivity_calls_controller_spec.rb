...
RSpec.describe LackOfConnectivityCallsController, type: :controller do
  fixtures :all
  ...
  describe "GET new" do
    ...
    context "for an authenticated nurse" do
      ...
      it "sets the lack_of_connectivity_call" do
        sign_in_user nurse

        get :new, participant_id: participant.id, locale: locale

        expect(assigns(:lack_of_connectivity_call))
          .to be_instance_of LackOfConnectivityCall
        expect(assigns(:lack_of_connectivity_call).participant)
          .to eq participant
      end
    end
  end
  ...
  describe "POST create" do
    ...
    context "for an authenticated nurse" do
      context "when successful" do
        it "creates a new LackOfConnectivityCall" do
          sign_in_user nurse

          expect do
            post :create, participant_id: participant.id,
                          lack_of_connectivity_call: valid_params,
                          locale: locale
          end.to change {
            LackOfConnectivityCall.where(participant: participant).count
          }.by(1)
        end
      ...
      context "when unsuccessful" do
        it "sets the flash alert" do
          ...
          post :create, participant_id: participant.id, locale: locale,
                        lack_of_connectivity_call: invalid_params
