...
RSpec.describe ScheduledTaskCancellationsController, type: :controller do
  fixtures :all
  ...
  let(:valid_params) { { explanation: "asdf" } }
    ...
    context "for an unauthenticated request" do
      before do
        post :create, participant_id: participant.id, locale: locale,
                      task_id: rand, scheduled_task_cancellation: valid_params
      end
    ...
    context "for an authenticated nurse" do
      ...
      context "when the Participant isn't found" do
        before do
          ...
          post :create, participant_id: -1, locale: locale, task_id: rand,
                        scheduled_task_cancellation: valid_params
        end
      ...
      context "when the Task isn't found" do
        before do
          ...
          post :create, participant_id: participant.id, locale: locale,
                        task_id: -1, scheduled_task_cancellation: valid_params
        end
