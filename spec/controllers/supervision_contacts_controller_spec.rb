...
RSpec.describe SupervisionContactsController, type: :controller do
  fixtures :all
  ...
  describe "GET new" do
    ...
    context "for an authenticated nurse" do
      ...
      it "sets the supervision_contact" do
        sign_in_user nurse_supervisor

        get :new, nurse_id: nurse.id, locale: locale

        expect(assigns(:supervision_contact))
          .to be_instance_of SupervisionContact
        expect(assigns(:supervision_contact).nurse)
          .to eq nurse
      end
    end
  end
  ...
  describe "POST create" do
    ...
    context "for an authenticated nurse" do
      context "when successful" do
        it "creates a new SupervisionContact" do
          sign_in_user nurse_supervisor

          expect do
            post :create, nurse_id: nurse.id,
                          supervision_contact: valid_params,
                          locale: locale
          end.to change {
            SupervisionContact.where(nurse: nurse).count
          }.by(1)
        end
      ...
      context "when unsuccessful" do
        it "sets the flash alert" do
          ...
          post :create, nurse_id: nurse.id, locale: locale,
                        supervision_contact: invalid_params
